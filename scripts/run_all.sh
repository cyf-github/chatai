...
echo "Ports: Service A=50051, B=50052, C=50053, Gateway=8000, Gradio=7860"
echo ""

# Start backends in background
echo "[1/5] Starting Service A (OpenAI) on 50051..."
python -m services.openai_service.server 50051 &
PID_A=$!

echo "[2/5] Starting Service B (Qwen) on 50052..."
python -m services.qwen_service.server 50052 &
PID_B=$!

echo "[3/5] Starting Service C (Doubao) on 50053..."
python -m services.doubao_service.server 50053 &
PID_C=$!

echo "[4/5] Starting Gateway on 8000..."
uvicorn gateway.main:app --host 0.0.0.0 --port 8000 &
PID_GW=$!

sleep 2

echo "[5/5] Starting Gradio frontend on 7860..."
python -m frontend.app

# If Gradio exits, kill background processes
kill $PID_A $PID_B $PID_C $PID_GW 2>/dev/null || true
